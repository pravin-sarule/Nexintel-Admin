...
//       {/* Add your prompt management content here */}
//     </div>
//   );
// };
...
const PromptManagement = () => {
  ...
  // API Base URL
  const API_BASE_URL = 'https://nexintel-admin.onrender.com/api/secrets';
  ...
  const fetchPrompts = async (includeValues = false) => {
    ...
    try {
      ...
      const transformedData = response.data.map(prompt => ({
        ...
        value: prompt.value || null, // Secret value if fetched
      }));
  ...
  const fetchSecretValue = async (promptId) => {
    ...
    try {
      ...
      // Update the prompt with the fetched value
      const updatedPrompt = {
        ...selectedPrompt,
