...
const LoginPage = ({ setAuthStatus }) => { // Added setAuthStatus prop
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://nexintel-admin.onrender.com/api/auth/login',
        { email, password },
        {
          headers: {
      ...
      setTimeout(() => {
        setAuthStatus(true); // Call setAuthStatus to update authentication state in App.jsx
        navigate('/dashboard');
      }, 1000);
    } catch (err) {
